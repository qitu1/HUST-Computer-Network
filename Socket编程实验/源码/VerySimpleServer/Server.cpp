...
//判断字符串后缀
bool str_suffix(const string& str, const string& suffix){
...
string retrun_content_type(const string& file_path) {
	if (str_suffix(file_path, ".html")) { return "text/html"; }
	else if (str_suffix(file_path, ".css")) { return "text/css"; }
	else if (str_suffix(file_path, ".gif")) { return "image/gif"; }
	else if (str_suffix(file_path, ".jpg") || str_suffix(file_path, ".jpeg")) { return "image/jpeg"; }
	else if (str_suffix(file_path, ".png")) { return "image/png"; }
	else if (str_suffix(file_path, ".js")) { return "application/javascript";}
	else {
		return "application/octet-stream"; // 默认二进制类型    
	}
}

void req_response(SOCKET sessionSocket, const string& document_root, char recvBuf[], int rtn) {
	...
	// 解析HTTP请求
	istringstream req_stream(string(recvBuf, rtn));
	string req_line;
	getline(req_stream, req_line);
	istringstream request_line_stream(req_line);
	...
	cout << "HTTP Request: " << req_line << endl;
	...
	if (method != "GET") {
		...
		// 这里创建一个新的 file_path 变量，避免之前的 file_stream 变量的干扰
		string file_path = document_root + "/501.html"; // 设置 501 页面路径
		ifstream file_stream(file_path.c_str(), ios::binary); // 这里直接打开 501.html
		//cout << "File_Path: " << file_path << endl;
		// 检查文件是否成功打开
		if (!file_stream) {
			cout << "501.html file not found." << endl; // 输出调试信息
			return; // 提前返回
		}
		...
		response_header << "Content-type: " << retrun_content_type(file_path) << "\r\n";
	...
	// 403 拒绝处理
	if (file_path == "D:/大三上/计网实验/模块一/VerySimpleServer/private.png")
	{
		cout << "403 Forbidden" << endl;


		// 这里创建一个新的 file_path 变量，避免之前的 file_stream 变量的干扰
		string file_path = document_root + "/403.html"; // 设置 403 页面路径
		ifstream file_stream(file_path.c_str(), ios::binary); // 打开 403.html
		//cout << "File_Path: " << file_path << endl;
		// 检查文件是否成功打开
		if (!file_stream) {
			cout << "403.html file not found." << endl; // 输出调试信息
			return; // 提前返回
		}
...
void main() {
	...
	*/
	...
	FILE* config_file = fopen("D:/大三上/计网实验/模块一/VerySimpleServer/config.txt", "r"); // 使用fopen打开配置文件
	...
	cout << "Port Number: " << port << std::endl;
	cout << "Home Directory: " << document_root << endl;
	...
	else { printf("Socket create failed!\n"); }
	...
	else printf("Soket bind failed :(\n");

	//监听
	rtn = listen(srvSocket, 5);
	if (rtn != SOCKET_ERROR)
		printf("Socket listen Ok!\n");
	else printf("Soket listen failed :(\n");
		...
		if (nTotal > 0) {
			...
			if (FD_ISSET(sessionSocket, &rfds)) {
				...
				if (rtn > 0) {
					...
					req_response(sessionSocket, document_root, recvBuf, rtn);
					request_over = false;
				}
